...
MULTIPLE CONDITIONS: 
    WHERE (field_1 > #01/01/2000# AND field_1 < #01/01/2001#) OR field_2 = 100 OR field_3 = 'string value'", "Filtering Query Syntax Examples");
            ...
            if (AppSettings.ReadingEngine == ParquetEngine.Default)
            {
                task = Task.Run(() =>
                {
                    using (var parquetReader = ParquetReader.OpenFromFile(this.OpenFilePath, new ParquetOptions() { TreatByteArrayAsString = true }))
                    {
                        ...
                        PassDetailsToASeparateTable(ref result);
        ...
        private void PassDetailsToASeparateTable(ref DataTable result)
        {
            try
            {
                DtDetails = result.Clone();
                foreach (DataRow drtableOld in result.Rows)
                {
                    DtDetails.ImportRow(drtableOld);
                }

                //Delete all columns except details
                for (int i = 0; i < DtDetails.Columns.Count - 1; i++)
                    DtDetails.Columns.RemoveAt(i);

                result.Columns.Remove("Details");
            }
            catch (Exception e)
            {
                Console.WriteLine("The parquet file does not have a column named 'Details'");
            }
        }
        ...
        private void ReadDataBackgroundWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                if (e.Error == null)
                {
                    ...
                    //Show details of the first row
                    DataGridViewCellEventArgs cellEvArgs = new DataGridViewCellEventArgs(0, 0);
                    this.mainGridView_CellClick(mainGridView, cellEvArgs);
                }
                else
                    throw e.Error;
            }
        ...
        {
            if (mainGridView != null && mainGridView.Rows.Count > 0)
            {
                //Highlight entire row
                DataGridViewRow row = mainGridView.Rows[e.RowIndex];
                int colGlobalKeyIdx = row.Cells["GlobalKey"].ColumnIndex;
                row.DefaultCellStyle.BackColor = Color.Aquamarine;

                //Get details in this row
                string globalKey = row.Cells[colGlobalKeyIdx].Value.ToString();

                int i = 0;
                DataRow rowWithdetails = DtDetails.Rows[i];
                while (rowWithdetails["GlobalKey"].ToString() != globalKey)
                {
                    i++;
                    rowWithdetails = DtDetails.Rows[i];
                }

                DataTable details = UtilityMethods.JsonContentToDataTable(rowWithdetails["Details"].ToString());
                detailGridView.DataSource = details;
            }

        }

        private void mainGridView_CellLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (mainGridView != null && mainGridView.Rows.Count > 0)
            {
                DataGridViewRow row = mainGridView.Rows[e.RowIndex];
